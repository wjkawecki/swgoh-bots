...
export default class CourtOfLaw {
  ...
  listenToMessages() {
    this.Client.on('message', msg => {
      ...
      if (command === 'help' && msg.member.roles.has(this.config.roles.member)) this.helpReply(msg);
      ...
      switch ((args[0] || '').toLowerCase()) {
        ...
        case 'echo':
          if (msg.member.roles.has(this.config.roles.officer)) this.sendEcho(msg, (args[1] || '').toLowerCase());
          break;

        case 'remove':
        case 'delete':
          if (msg.member.roles.has(this.config.roles.officer)) this.deleteCheckById(msg, (args[1] || '').toLowerCase());
          break;

        case 'all':
          if (msg.member.roles.has(this.config.roles.officer)) this.printCourt(msg, 'all');
          break;
  ...
  listenToReactions() {
    this.Client.on('messageReactionAdd', (messageReaction, user) => {
      if (
        ...
        if (this.hasReaction(messageReaction, 'courtOfLaw_1d')) this.addCheck(messageReaction, user, 1 * 24 * 60 * 60 * 1000);

        if (this.hasReaction(messageReaction, 'courtOfLaw_2d')) this.addCheck(messageReaction, user, 2 * 24 * 60 * 60 * 1000);

        if (this.hasReaction(messageReaction, 'courtOfLaw_3d')) this.addCheck(messageReaction, user, 3 * 24 * 60 * 60 * 1000);

        if (this.hasReaction(messageReaction, 'courtOfLaw_4d')) this.addCheck(messageReaction, user, 4 * 24 * 60 * 60 * 1000);

        if (this.hasReaction(messageReaction, 'courtOfLaw_5d')) this.addCheck(messageReaction, user, 5 * 24 * 60 * 60 * 1000);

        if (this.hasReaction(messageReaction, 'courtOfLaw_6d')) this.addCheck(messageReaction, user, 6 * 24 * 60 * 60 * 1000);

        if (this.hasReaction(messageReaction, 'courtOfLaw_7d')) this.addCheck(messageReaction, user, 7 * 24 * 60 * 60 * 1000);

        if (this.hasReaction(messageReaction, 'courtOfLaw_8d')) this.addCheck(messageReaction, user, 8 * 24 * 60 * 60 * 1000);

        if (this.hasReaction(messageReaction, 'courtOfLaw_9d')) this.addCheck(messageReaction, user, 9 * 24 * 60 * 60 * 1000);
  ...
  addCheck(messageReaction, user, timeout) {
    ...
    if (reactions.get('▶') && reactions.get('▶').users.find(user => user.bot)) {
      messageReaction.message.react('🚫').then(() =>
        setTimeout(() => {
    ...
    if (messageIndex === -1) {
      ...
      helpers.updateJSON(this.config, 'courtOfLaw', this.data, () => {
        messageReaction.message.react('▶').then(() => this.main());
      });
    }
  }

  deleteCheck(messageOrMessageReaction, user) {
    const messageIndex = this.data.messages
      .map(message => message.id)
      .indexOf(messageOrMessageReaction.id || messageOrMessageReaction.message.id);

    if (messageIndex > -1) {
      const message = this.data.messages[messageIndex];

      console.log(
        `${this.config.name}: CourtOfLaw deleteCheck - ${user.username} removed ${messageOrMessageReaction.id || message.emojiName}`
      );
...
\`-court 10/1/2018 10/20/2018\``);
  ...
  printCourt(msg, fromString, toString) {
    ...
    if (fromString === 'all') {
      dateFrom = null;
      dateTo = null;
    } else if (fromString === 'this' || fromString === 'current' || fromString === 'currentmonth' || fromString === 'thismonth') {
      dateFrom = new Date(new Date(now.getFullYear(), now.getMonth(), 1).getTime());
      dateTo = null;
    } else if (fromString === 'last' || fromString === 'previous' || fromString === 'lastmonth' || fromString === 'previousmonth') {
    ...
    this.fetchSlackers(dateFrom, dateTo).then(({ slackers, lastMessageTimestamp }) =>
  ...
  fetchSlackers(timestampFrom, timestampTo, messageId) {
    ...
    return new Promise(resolve => {
      ...
      const channel = this.config.DEV ? this.Client.channels.get(this.config.channels.court_of_law) : this.channels.court_of_law;

      if (messageId) options.before = messageId;
      ...
      channel.fetchMessages(options).then(messages => {
        ...
        if (reachedTimestampFrom || messages.size < options.limit) {
          ...
          this.slackers = new Map(
            [...this.slackers].sort((a, b) => b[1].mentionCount - a[1].mentionCount || a[1].displayName.localeCompare(b[1].displayName))
          );

          resolve({
            slackers: this.slackers,
            lastMessageTimestamp: messages.size < options.limit ? messages.last().createdTimestamp : null
          });
        } else {
  ...
  scheduleCourt() {
    ...
    this.timeouts.court.push(
      setTimeout(() => {
        ...
        this.fetchSlackers(dateFrom, dateTo).then(({ slackers }) => this.printSlackers(null, dateFrom, dateTo, slackers));
      }, timeout)
    );
        ...
        setTimeout(
          () => {
            ...
            if (channel.members && channel.members.has(this.Client.user.id)) {
              ...
              channel.fetchMessage(message.id).then(msg => {
  ...
  sendDM(sendTo = 'slacker', msg, slacker, message) {
    ...
    this.guild.members
      ...
      .then(channel => {
        if (sendTo === 'author') {
          channel.send(
            `Hello ${message.authorUsername}.

•    You have mentioned **${slacker.values().next().value.displayName}** in \`#court-of-law\` ${helpers.getReadableTime(
              message.timeCheck - message.timeAdded,
              this.config.DEV
            )} ago.
...
•    Jump to that message: ${msg.url}`,
          ...
          channel.send(
            `Hello ${message.userUsername}.

•    ${message.authorUsername} has mentioned **${
              slacker.values().next().value.displayName
            }** in \`#court-of-law\` ${helpers.getReadableTime(message.timeCheck - message.timeAdded, this.config.DEV)} ago.
...
•    Jump to that message: ${msg.url}`,
          ...
          channel.send(
            `Hello ${slacker.displayName}.

•    You have been mentioned by ${message.authorUsername} in \`#court-of-law\` ${helpers.getReadableTime(
              message.timeCheck - message.timeAdded,
              this.config.DEV
            )} ago.
...
•    ${slackers.size} ${slackers.size === 1 ? 'member' : 'members'} of ${this.guild.roles.get(this.config.roles.member).name} ${
        slackers.size === 1 ? 'was' : 'were'
      } mentioned during that time${slackers.size > 0 ? ':' : '.'}
${[...slackers].map(slacker => `      - <@${slacker[1].id}> ${slacker[1].mentionCount}x`).join('\n')}`,
  ...
  clearTimeouts(key) {
    key &&
      this.timeouts[key] &&
      this.timeouts[key].forEach(timeout => {
