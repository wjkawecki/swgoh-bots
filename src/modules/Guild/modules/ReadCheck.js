...
export default class ReadCheck {
  ...
  listenToMessages() {
    this.Client.on('message', msg => {
      ...
      if (command === 'dm' && msg.member.roles.has(this.config.roles.member)) this.sendDM(msg, command);
      ...
      switch ((args[0] || '').toLowerCase()) {
        ...
        default:
          this.manualCheck(msg, (args[0] || '').toLowerCase());
      }
    });
  }
  ...
  sendEcho(msg, command) {
    msg.channel.send(msg.content.split(`${command} `).pop()).then(message => {
  ...
  sendDM(msg, command) {
    ...
    user.createDM().then(channel => channel.send(msg.content.split(`${command} `).pop()));
  }
  ...
  manualCheck(msg, messageId) {
    msg.channel.fetchMessage(messageId).then(message =>
      this.performCheck({
        manual: true,
        triggerMesssage: msg,
        message
  ...
  deleteCheck(messageReaction, user) {
    ...
    if (messageIndex > -1) {
      ...
      if (this.config.DEV) console.log('deleteCheck', message.userId, user.id, message.emojiName, messageReaction.emoji.name);
...
Once scheduled, ReadCheck will run through all people @mentioned in that message after certain time and list those who didn't react to that message with an emoji (emoji type doesn't matter).`);
  ...
  scheduleChecks() {
    ...
    this.data.messages.forEach((message, messageIndex) => {
      ...
      this.timeouts.push(
        setTimeout(
          this.performCheck({ message, messageIndex }),
  ...
  performCheck({ manual, triggerMesssage, message, messageIndex }) {
    const channel = triggerMesssage.channel || this.Client.channels.get(message.channelId);
      ...
      channel.fetchMessage(message.id).then(msg => {
        ...
        Promise.all(reactionPromises).then(() => {
          ...
          if (slackers.size) {
            ...
            if (manual) {
              this.sendDMReport({ triggerMesssage, slackers, message });
            } else {
  ...
  sendDMReport({ triggerMesssage, slackers, message }) {
    this.guild.members
      .get(triggerMesssage.author.id)
      .createDM()
      .then(channel => {
        channel
          .send(
            `__**ReadCheck Report**__

•    ${slackers.size} ${slackers.size > 1 ? `people haven't` : `person hasn't`} reacted to the tracked message in ${helpers.getReadableTime(
...
•    Jump to that message: ${message.url}`,
            { split: true }
          )
          .then(() => triggerMesssage.delete());
      });
  }
  ...
  sendReport(msg, slackers, message, messageIndex) {
    ...
    const sendReply = () => {
      ...
      channel
        .send(
          `__**ReadCheck Report${
            message.shouldRepeat ? ` (repeats every ${helpers.getReadableTime(message.timeout, this.config.DEV)})` : ''
          }**__
...
/cc ${message.authorId !== message.userId ? `<@${message.authorId}>, <@${message.userId}>` : `<@${message.authorId}>`}`,
          { split: true }
        )
...
•    You haven't reacted to a tracked message in ${helpers.getReadableTime(
                message.timeCheck - message.timeAdded,
                this.config.DEV
              )}. Please make sure you read it and react to it with any emoji.
