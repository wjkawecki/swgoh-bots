...
export default class DailyActivities {
	constructor(Client) {
		console.log(`CloneSergeant.DailyActivities ready${DEV ? ' (DEV mode)' : ''}`);
	...
	async main() {
		try {
			console.log('CloneSergeant.DailyActivities.main()');
...
:heavy_multiplication_x:  **Save** Normal Energy`
	...
	scheduleReminder(manualReminder = false) {
		...
		if (manualReminder) {
			...
			console.log(`CloneSergeant.DailyActivities.scheduleReminder(): ${this.getReadableTime(diff)} to reset`);

			let reminderDiff = diff - (remindHoursBefore * 60 * 60 * 1000);

			if (reminderDiff > 0) {
				setTimeout(() => {
					this.channels.guild_lounge.send(`<@&${roles.member}> ${remindHoursBefore} hours left to get your :six::zero::zero: daily tickets. Go grab them now!`);
				}, reminderDiff);
			}
...
\`-help\` - this is what you are reading right now.`);
	...
	async clearChannel(channel, removeAll = false) {
		console.log(`CloneSergeant.DailyActivities.clearChannel()`);
